...
from neuro_hls.ImplementationManager import ImplementationManager
...
class NIRToCppParser:
    ...
    def get_model_config_from_nir(self):
        """
        Cria um ModelConfig a partir do gráfico NIR.
        Retorna o ModelConfig e o input_shape inferido.
        """
        from neuro_hls.ModelConfig import ModelConfig, DenseLayerConfig
        ...
        # Inicializa o ModelConfig
        model_config = ModelConfig()
        
        # Rastreia o número de características atual (após flatten, etc.)
        current_features = int(np.prod(input_shape))
        ...
        for node_name in self.execution_order:
            ...
            if node_type == 'Flatten':
                # Flatten não adiciona camada, apenas atualiza current_features
                # (já está calculado como prod de input_shape)
                pass
                ...
                # Adiciona camada Dense ao ModelConfig
                dense_layer = DenseLayerConfig(n_inputs=in_features, n_neurons=out_features)
                model_config.add_layer(dense_layer)
                
                # Atualiza current_features para a próxima camada
                current_features = out_features
                output_size = out_features
        
        return model_config, input_shape, output_size

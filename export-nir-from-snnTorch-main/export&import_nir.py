...
if __name__ == "__main__":
    ...
    # 4) Carregar .nir -> voltar à rede PyTorch e testar
    from snntorch.import_nir import import_from_nir

    # Lê o grafo NIR salvo
    nir_graph_loaded = nir.read("csnn_mnist.nir")

    for node in nir_graph_loaded.nodes.values():
        if isinstance(node, nir.Flatten):
            # se o export colocou start_dim=0 (sem batch), volta para 1 (preserva batch)
            if getattr(node, "start_dim", 1) == 0:
                node.start_dim = 1
            # normalmente end_dim já é -1; não precisa mexer


    # Converte NIR -> snnTorch/PyTorch nn.Module
    net_import = import_from_nir(nir_graph_loaded).to(device)
    net_import.eval()

    # Forward temporal genérico: lida com (spk, mem) OU (out, state) OU só out
    def forward_pass_generic(model, num_steps, data):
        spk_rec = []
        utils.reset(model)
        for _ in range(num_steps):
            out = model(data)
            # GraphExecutor pode retornar tuplo; pegue só o primeiro elemento como "spk"
            if isinstance(out, tuple):
                out = out[0]
            spk_rec.append(out)
        return torch.stack(spk_rec)  # [T, B, 10] no seu caso

    # Métrica de acurácia por taxa de disparos (igual à que você já usa)
    @torch.no_grad()
    def batch_accuracy_import(loader, model, num_steps):
        total = 0
        acc_sum = 0.0
        model.eval()
        for data, targets in loader:
            data, targets = data.to(device), targets.to(device)
            spk_rec = forward_pass_generic(model, num_steps, data)  # [T,B,10]
            # mesma ce_rate/accuracy_rate: conta picos por classe ao longo do tempo
            acc_sum += SF.accuracy_rate(spk_rec, targets) * spk_rec.size(1)
            total  += spk_rec.size(1)
        return acc_sum / total

    # Avalia a rede importada
    test_acc_import = batch_accuracy_import(test_loader, net_import, num_steps)
    print(f"Acurácia (rede importada do NIR): {test_acc_import*100:6.2f}%")

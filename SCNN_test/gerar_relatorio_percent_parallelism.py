...
def _sweep_chart(records, charts_dir: Path, panels, stem: str) -> Path | None:
    labels, series, reference, _ = _chart_series(records)
    if not series:
        return None
    charts_dir.mkdir(parents=True, exist_ok=True)
    # O sweep e indexado pela requisicao em toda parte -- eixo, tabelas e
    # texto -- entao a legenda nao repete o contador de elementos resolvido.
    figure = sweep_figure(
...
def cycles_per_logical_step(record: ImplementationRecord) -> tuple[float, str] | None:
    """Cycles the implementation spends on one time step, with its source.
...
def render_step_latency_table(records: list[ImplementationRecord]) -> list[str]:
    """Compare cycles per time step across backends, against the event reference."""
    medidas = []
    for record in records:
...
def build_report(
    ...
    lines += [
        ...
        "time step count, which is available only where synthesis reports a "

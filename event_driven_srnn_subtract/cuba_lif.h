...
typedef enum {
    CUBA_LIF_RESET_BY_VALUE = 0,
    CUBA_LIF_RESET_BY_SUBTRACTION = 1
} cuba_lif_reset_mode_t;
 ...
 * @param reset_mode Modo de reset: por valor (v_reset) ou por subtracao (v_t - v_th).
...
inline current_t update_cuba_lif_neuron(
    ...
    cuba_lif_reset_mode_t reset_mode = CUBA_LIF_RESET_BY_VALUE
) {
    ...
    *i_syn = (current_t)(i_t + delta_i);
    ...
    if (v_t >= (accum_t)v_th) {
        if (reset_mode == CUBA_LIF_RESET_BY_SUBTRACTION) {
            *v_mem = (voltage_t)(v_t - (accum_t)v_th);
        } else {
            *v_mem = v_reset;
        }
        return (current_t)1;

#!/usr/bin/env python3
"""Generate the time-driven percent-parallel MLP components of the sweep.
...
COMPONENTS_DIR = REPO_ROOT / "MLP_test"
MANIFEST = COMPONENTS_DIR / "percent_parallelism_component_manifest.json"

SOURCE_NIR = COMPONENTS_DIR / "trained_snn_n-mnist.nir"
METADATA = COMPONENTS_DIR / "trained_snn_n-mnist.metadata.json"
DATASET = COMPONENTS_DIR / "n-mnist-32-steps-medoids.pt"
...
def project_dir(units: int) -> Path:
    return COMPONENTS_DIR / f"hls_time_driven_percent_w{units:04d}"
...
def generate(units: int, *, force: bool = False) -> Path:
    ...
    model = hls.read_nir_file(str(SOURCE_NIR), str(METADATA))
...
def register(units: int) -> None:
    ...
    components.sort(
        key=lambda c: (
            (c.get("plans") or [{}])[0].get("processing_elements", float("inf")),
            c["name"],
        )
    )
